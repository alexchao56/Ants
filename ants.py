...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        #create a method that will iterate through each of the places
        #and then return whether there is a bee in there or not
        # python -m unittest -v tests.TestProblemB4
        count = 0
        place = self.place
        while (len(place.bees) == 0) and (place is not hive) and (count <= self.max_range) and (count >= self.min_range):
            count += 1
